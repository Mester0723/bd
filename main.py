...
/projects - используй для просмотра всех своих проектов
...
def addtask_command(message):
    bot.send_message(message.chat.id, "✏️ Введите название проекта:")
...
def callback_project(message, data, statuses):
    ...
    statuses_clean = [x.strip().lower() for x in statuses]
    if message.text == cancel_button:
        cansel(message)
        return
    if status not in statuses_clean:
        bot.send_message(message.chat.id, "🚫 Ты выбрал статус не из списка, попробуй еще раз!)", reply_markup=gen_markup(statuses))
        bot.register_next_step_handler(message, callback_project, data=data, statuses=statuses)
        return
    original_status = statuses[statuses_clean.index(status)]
    status_id = manager.get_status_id(original_status)
    if status_id is None:
        bot.send_message(message.chat.id, "🚫 Не удалось найти статус в базе. Обратитесь к администратору.")
        return
...
def set_skill(message, project_name, skills):
    ...
    if skill not in skills:
        bot.send_message(message.chat.id, '🚫 Видимо, ты выбрал навык. не из спика, попробуй еще раз!)\n' \
                                          '🧩 Выбери навык', reply_markup=gen_markup(skills))
        bot.register_next_step_handler(message, set_skill, project_name=project_name, skills=skills)
        return
...
def delete_project(message, projects):
    ...
    bot.send_message(message.chat.id, f'🗑️ Проект {project} удален!')
...
def update_project_step_2(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(message.chat.id, "⁉️ Что-то пошло не так!)\n" \
                                          "✏️ Выбери проект, который хочешь изменить еще раз:", reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, update_project_step_2, projects=projects )
        return
...
def text_handler(message):
    ...
    if project in projects:
        ...
        return
    bot.reply_to(message, "⁉️ Тебе нужна помощь?")
    info(message)
